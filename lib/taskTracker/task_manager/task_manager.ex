defmodule TaskTracker.TaskManager do
  ...
  def list_tasks_of_user(user_id) do
    Repo.all(from t in Task, where: t.user_id == ^user_id)
    |> Repo.preload(:user)
  end

  def list_tasks_of_underlings(user_id) do
    Repo.all(from t in Task, join: u in User, on: t.user_id == u.id, where: u.mgr_id == ^user_id, select: t)
    |> Repo.preload(:user)
  end
  ...
  def list_tblocks_of_task(task_id) do
